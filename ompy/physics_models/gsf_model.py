...
class GSF_model(Model):
    ...
    def setup_prior(self, priors: dict) -> int:
        """ Extract and setup of the prior function.
        Args: priors (dict) containing the priors of each parameter in the
            model. If all priors uses a normal distribution the class will
            optimize by vectorizing the call to the prior method.
            Example of the structure:
            ```
            priors = {'GDR':
                        {'mean': {'kind': 'normal', 'mu': 17.1, 'sigma': 1.8},
                         'width': {'kind': 'normal', 'mu': 6.3, 'sigma': 0.9},
                         'size': {'kind': 'truncnorm', 'mu': 1202,
                                  'sigma': 150, 'lower': 0, 'upper': np.inf}},
                      'PDR':
                        {'mean': {'kind': 'normal', 'mu': 10.2, 'sigma': 1.0},
                         'width': {'kind': 'normal', 'mu': 1.01, 'sigma': 0.1},
                         'size': {'kind': 'truncnorm', 'mu': 20.1,
                                  'sigma': 0.4, 'lower': 0, 'upper': 50}},
                      ...
                    }
            ```
            Note that if 'kind' is not given, a normal distribution is assumed.
            If there is a parameeter named 'T' it will assume this to be
            a parameter that is not free if `'T': None` in the dict.
        """
        n_param = 0
    ...
    def mean(self, E):
        if self.need_T:
            param = list(self.prior(np.ones(self.N_free)*0.5))

            sig = list(dict(signature(self.__call__).parameters).keys())

            param.insert(self.T_idx, 0.8)
            return self.__call__(E, *param)
        else:
            return self.__call__(E, *self.prior(np.ones(self.N_free)*0.5))
